...
function createNewCustomTruthTable() {
     ...
     function regenerateTableArguments() {
          var ncolumns = Object.keys(CUSTOM_TABLE.ARGS).length
          ...
          for (let lIndex = 0; lIndex < ncolumns; lIndex++) {
               let newArg = String.fromCharCode(Object.keys(CUSTOM_TABLE.ARGS).length + 65);
               CUSTOM_TABLE.ARGS[newArg] = Array(ncolumns)
               for (const [i, row] of combinations.entries()) {
                    CUSTOM_TABLE.ARGS[newArg][i] = row[lIndex]
               }
          }
     }
     ...
     function setupTableButtons() {
          ...
          addArgumentButton.onclick = function() { 
               let args = Object.keys(CUSTOM_TABLE.ARGS);

               if ( args.length >= 6 ) { return } 

               let newArg = String.fromCharCode(args.length + 65);
               CUSTOM_TABLE.ARGS[newArg] = Array(CUSTOM_TABLE.ARGS[args[0]].length).fill(-1);
               regenerateTableArguments()
               updateTable();

               if ( Object.keys(CUSTOM_TABLE.ARGS).length >= 6 ) { addArgumentButton.disabled = true } 
          }

          let addFunctionButton = document.getElementById(`${uniqueTableId}-button-addfunc`)
          addFunctionButton.onclick = function() {
               let funcs = Object.keys(CUSTOM_TABLE.FUNCS);
               let newFunc = "not"
               CUSTOM_TABLE.FUNCS.push([newFunc, Array(CUSTOM_TABLE.ARGS[Object.keys(CUSTOM_TABLE.ARGS)[0]].length).fill(-1)])// = Array(CUSTOM_TABLE.ARGS[Object.keys(CUSTOM_TABLE.ARGS)[0]].length).fill(0);
               updateTable();
          }
     ...
     function updateSelectInput(hisid, hisvalue) {
     ...
     function setupSelectors(){
          for (element of document.getElementsByClassName(`function-selector-${uniqueTableId}`)){
               // console.log(CUSTOM_TABLE.FUNCS[Number(element.dataset.column)][0])
               element.value = CUSTOM_TABLE.FUNCS[Number(element.dataset.column)][0]
               updateSelectInput(`formula-input-first-${uniqueTableId}-${element.dataset.column}`, element.value)
               updateSelectInput(`formula-input-second-${uniqueTableId}-${element.dataset.column}`, element.value)
               element.onchange = function(){
                    CUSTOM_TABLE.FUNCS[element.dataset.column][0] = String(element.value)
                    updateSelectInput(`formula-input-first-${uniqueTableId}-${element.dataset.column}`, element.value)
                    updateSelectInput(`formula-input-second-${uniqueTableId}-${element.dataset.column}`, element.value)
               }
          }
     ...
     function updateTable() {
          ...
          Object.keys(CUSTOM_TABLE.ARGS).forEach((arg, index) => {
          ...
          Object.keys(CUSTOM_TABLE.FUNCS).forEach((func, index) => {
               table += `<th>${generateDefaultFunctions(index)}</th>`;
          });
 
          table += "</tr></thead><tbody>";
 
          for (let i = 0; i < CUSTOM_TABLE.ARGS.A.length; i++) {
               table += "<tr>";

               Object.keys(CUSTOM_TABLE.ARGS).forEach(arg => {
                    table += `<td><input data-notinputing=true readonly class="table_input" type="number" value="${CUSTOM_TABLE.ARGS[arg][i]}" min=0 max=1 data-truevalue="${CUSTOM_TABLE.ARGS[arg][i]}"></td>`;
               });

               Object.values(CUSTOM_TABLE.FUNCS).forEach(func => {
